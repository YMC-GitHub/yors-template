...
import { viteplugify as cooktstodist } from "@yors/vite-plugin-make-type";
import {
  makeBannerText,
  stdUmdName,
...
const jslibformats: LibraryFormats[] = ["cjs", "es"];
let typeoption = {
  jsfileOutDir,
  tsTypeOutDir,
  jsfileSrcDir,
...
let plugins = [cooktstodist(typeoption), minijsindist({ banner: bannerText })];
